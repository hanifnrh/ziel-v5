    ...
    return (React.createElement("div", null,
        ...
        React.createElement("section", { className: "pt-16 lg:py-28 z-10" },
            ...
            React.createElement("div", { className: "w-full relative flex flex-col items-start px-8 md:px-20 2xl:px-52 pb-20 bg-background", id: "projects" },
                React.createElement("div", { className: "w-full grid grid-cols-1 items-start mt-5" }, posts.length > 0 ? (posts.map(function (post, index) { return (React.createElement(link_1["default"], { key: index, href: "/blog/" + post.slug, className: "w-full group border-zinc-500/20 border-t-1 py-10 body grid grid-cols-1 sm:grid-cols-[1fr_3fr] gap-4 lg:gap-6 xl:gap-8 justify-center items-start text-zinc-200" },
                    ...
                    React.createElement("div", { className: "w-full h-full flex flex-col gap-4 justify-between" },
                        React.createElement("div", { className: "flex flex-col gap-2" },
                            React.createElement("div", { className: "body-light text-xs sm:text-sm text-zinc-200" }, formatDate(post.publishDate)),
                            React.createElement("h3", { className: "text-xl lg:text-3xl" }, post.title),
                            React.createElement("p", { className: "text-sm sm:text-md text-zinc-500 line-clamp-3" }, post.description)),
                        React.createElement("div", { className: "flex justify-between w-full" },
                            ...
                            React.createElement("div", { className: "flex body-light text-zinc-500 gap-2" }, (Array.isArray(post.tag) ? post.tag : post.tag.split(",")).map(function (tag, index) { return (React.createElement("p", { key: index, className: "bg-zinc-200/10 px-3 py-2 text-xs rounded-sm" }, tag.trim())); })))))); })) : (React.createElement("p", { className: "text-center text-zinc-500" }, "No posts available.")))))));
}
