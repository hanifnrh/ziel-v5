...
var navItems = [
    ...
    },
    {
        ...
        icon: React.createElement(lucide_react_1.FileUser, { className: "h-full w-full" })
    },
    {
        id: 4,
        label: "Guestbook",
        ...
        React.createElement("div", { className: "relative flex items-center" },
            ...
            navItems.map(function (item) {
                return item.href ? (React.createElement(link_1["default"], { key: item.id, href: item.href, scroll: true, onClick: function () { return setActiveTab(item.id); }, className: (activeTab === item.id
                        ? "text-purple-600"
                        : "text-zinc-500") + " group/dock relative h-10 w-10 p-3 text-sm transition-all duration-300 ease-in-out focus-visible:outline-none", style: {
